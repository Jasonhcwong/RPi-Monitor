...
parsed_info="$(echo $info | jq '.nettype, .height, .target_height, .incoming_connections_count, .outgoing_connections_count, .difficulty, .target, .release_name, .version_full' | tr '\n' ',' )$start_time,"
